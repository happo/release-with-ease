...
function fetchNpmOtp() {
  const isValidOtp = code => /^\d{6,8}$/.test(code);
  ...
  // 1. Explicit override
  if (process.env.NPM_OTP_COMMAND) {
    const code = tryCommand(process.env.NPM_OTP_COMMAND);
    if (code) {
  ...
  const candidateNames = ['npmjs.com', 'npm', 'npmjs'];
